...
prop_unsolved = prop.test(x = baltimore$total_unsolved, n = baltimore$total_homicides)
...
```{r}
props_function = function(dataframe, city_name){
  city = dataframe %>% 
...
props_all_cities = hom_data_summed %>% 
  mutate(
    prop_unsolved = map2(.x = ., .y = city_state, ~props_function(dataframe = .x, city_name = .y))
    ) %>% 
  unnest 
```
